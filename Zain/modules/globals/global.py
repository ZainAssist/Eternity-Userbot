...
from Zain.helper.PyroHelpers import get_ub_chats
from Zain.modules.basic.profile import extract_user, extract_user_and_reason
from Zain.database import gbandb as RAUSHAN
from Zain.database import gmutedb as Gmute
...
async def gban_user(client: Client, message: Message):
    ...
    if (await Zain.gban_info(user.id)):
...
async def ungban_user(client: Client, message: Message):
    ...
    try:
        if not (await Zain.gban_info(user.id)):
        ...
        for good_boi in ung_chats:
            try:
                ...
                er += 1
        await Zain.ungban_user(user.id)
        msg = (
            r"**\\#UnGbanned_User//**"
            f"\n\n**First Name:** [{user.first_name}](tg://user?id={user.id})"
            f"\n**User ID:** `{user.id}`"
...
if ok:
 ...
 async def globals_check(client: Client, message: Message):
    ...
    if not user_id:
        return
    if (await Zain.gban_info(user_id)):
        try:
