...
from Zain import app, CMD_HELP
from Zain.helper.PyroHelpers import ReplyCheck
from Zain.helper.utility import split_list
...
async def module_help(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        await message.edit("➻ ♥️꯭ ⃪꯭〬⃝‌ٖٖٖٖٖٖٜٖᬃ꯭ᵘᶠᶠ꯭؛ًٍَُّ𝅯𝅯۪ 𝐙꯭𝛂꯭⃛ιꪀ꯭ꯩ⃪𝅏꯭꯭꯭𝆭‌🩵ᬼᭂᭃ꯭꯭༉꯭᪳ ᴘᴀᴘᴀ ᴄᴏᴍɪɴɢ..😈")
        try:
            ...
            ac = PrettyTable()
            ac.header = False
            ac.title = "♥️꯭ ⃪꯭〬⃝‌ٖٖٖٖٖٖٜٖᬃ꯭ᵘᶠᶠ꯭؛ًٍَُّ𝅯𝅯۪ 𝐙꯭𝛂꯭⃛ιꪀ꯭ꯩ⃪𝅏꯭꯭꯭𝆭‌🩵ᬼᭂᭃ꯭꯭༉꯭᪳"
            ac.align = "l"
...
async def module_helper(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        ac = PrettyTable()
        ac.header = False
        ac.title = "Eternity-UserBot Plugins"
        ac.align = "l"
        for x in split_list(sorted(CMD_HELP.keys()), 2):
            ac.add_row([x[0], x[1] if len(x) >= 2 else None])
        await edit_or_reply(
    ...
    if help_arg:
        if help_arg in CMD_HELP:
            ...
            for x in commands:
                this_command += f"  •  **Command:** `.{str(x)}`\n  •  **Function:** `{str(commands[x])}`\n\n"
            this_command += "© @The_Eternity_Soul"
            await edit_or_reply(
                message, this_command, parse_mode=enums.ParseMode.MARKDOWN
            )
        else:
