...
def restart():
    os.execvp(sys.executable, [sys.executable, "-m", "Zain"])
...
async def upstream(client: Client, message: Message):
    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return
        await status.edit(
            "`[HEROKU]: Eternity-Userbot Deploy Update is in Progress...`"
        )
        ups_rem.fetch(ac_br)
...
add_command_help(
    "update",
    [
        ["update", "To see a list of the latest updates from Eternity-Userbot."],
