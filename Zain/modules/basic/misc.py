...
from Zain import START_TIME
from Zain import SUDO_USER
from Zain.helper.PyroHelpers import ReplyCheck
from Zain.modules.help import add_command_help
from Zain.modules.bot.inline import get_readable_time

alive_logo = ALIVE_PIC or "https://graph.org/file/0d88697ff8d448bfd3e3c-0632eff2d98dfdd765.jpg"

if ALIVE_TEXT:
   txt = ALIVE_TEXT
else:
    txt = (
        f"** ♥️꯭ ⃪꯭〬⃝‌ٖٖٖٖٖٖٜٖᬃ꯭ᵘᶠᶠ꯭؛ًٍَُّ𝅯𝅯۪ 𝐙꯭𝛂꯭⃛ιꪀ꯭ꯩ⃪𝅏꯭꯭꯭𝆭‌🩵ᬼᭂᭃ꯭꯭༉꯭᪳ **\n\n"
        f"❏ **⚜️𝐕ᴇʀsɪᴏɴ**: `2.1`\n"
        f"├• **⚜️𝐔ᴘᴛɪᴍᴇ**: `{str(datetime.now() - START_TIME).split('.')[0]}`\n"
        f"├• **⚜️𝐏ʏᴛʜᴏɴ**: `{python_version()}`\n"
        f"├• **⚜️𝐏ʏʀᴏɢʀᴀᴍ**: `{__version__}`\n"
        f"├• **⚜️𝐒ᴜᴘᴘᴏʀᴛ-𝐂ʜᴀᴛ**: [|| ˹Full On Masti˼ ||](https://t.me/+Mod-Itq-TwBlNjY1)\n"
        f"├• **⚜️𝐔ᴘᴅᴀᴛᴇs**: [About Zain](t.me/About_Zain)\n"
        f"└• **⚜️𝐌ᴀsᴛᴇʀ**: [Zain](t.me/Uff_Zainu)"        
    )
...
async def repo(bot: Client, message: Message):
    ...
    await asyncio.sleep(1)
    await message.edit("Here is repo: \n\n\n\nhttps://t.me/About_Zain & Give an ⭐")


@Client.on_message(filters.command("creator", ".") & filters.me)
async def creator(bot: Client, message: Message):
    await message.edit("https://t.me/+Mod-Itq-TwBlNjY1")
...
add_command_help(
    "start",
    [
        [".alive", "⚜️Check if the bot is alive or not.⚜️"],
        [".repo", "⚜️Display the repo of this userbot.⚜️"],
        [".creator", "⚜️Show the creator of this userbot.⚜️"],
        [".id", "⚜️Send id of what you replied to.⚜️"],
        [".up `or` .uptime", "⚜️Check bot's current uptime.⚜️"],
    ],
)
...
add_command_help(
    "restart",
    [
        [".restart", "You are retarded if you do not know what this does.⚜️"],
    ],
)
