...
from Zain.database.pmpermitdb import get_approved_users, pm_guard
import Zain.database.pmpermitdb as Zain
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Zain.set_block_message(Zain.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Zain.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await Zain.get_pm_settings()
    await Zain.allow_user(chat_id)
